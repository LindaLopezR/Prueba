...
export default SettingsRecognitions = () => {
  ...
  useEffect(() => {
    if (companyInfo) {
      setEmail(companyInfo.emailRecognitions ? companyInfo.emailRecognitions : '');
      setName(companyInfo.userRecognitions ? companyInfo.userRecognitions : '');
    }
  }, [companyInfo]);
  ...
  const onSubmit = () => {
    ...
    Meteor.call('settingsRecognitions', data, function(error, result) {
      setLoading(false);
      if (error) {
        setTitleModal('Error');
            ...
            <Card>
              <Card.Body>
                <Form.Group controlId="email">
                  <Form.Label>Correo para notificaciones de reconocimientos</Form.Label>
                  <Form.Control 
                    type="email"
                    value={email}
